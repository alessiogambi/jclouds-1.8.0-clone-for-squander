...
public class NodeMetadataImpl extends ComputeMetadataImpl implements
	...
	private final NodeMetadataStatus status;
	...
	public NodeMetadataImpl(String providerId, String name, String id,
			...
			Set<String> tags, @Nullable String group,
			@Nullable Hardware hardware, @Nullable String imageId,
			@Nullable OperatingSystem os, NodeMetadataStatus status,
			@Nullable String backendStatus, int loginPort,
			Iterable<String> publicAddresses,
			Iterable<String> privateAddresses,
			@Nullable LoginCredentials credentials, String hostname) {
		super(ComputeType.NODE, providerId, name, id, location, uri,
				userMetadata, tags);
		this.group = group;
	...
	protected ToStringHelper string() {
		...
		helper.add("group", getGroup()).add("imageId", getImageId())
				.add("os", getOperatingSystem())
				.add("status", formatStatus(this))
				.add("loginPort", getLoginPort())
		...
		helper.add("hardware", getHardware()).add("loginUser",
				credentials != null ? credentials.identity : null);
