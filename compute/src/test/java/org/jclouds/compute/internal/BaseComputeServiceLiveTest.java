...
public abstract class BaseComputeServiceLiveTest extends
	...
	public void testCorrectAuthException() throws Exception {
		...
		try {
			...
			context = newBuilder()
					.modules(
							ImmutableSet.of(getLoggingModule(),
	...
	public void testCorrectExceptionRunningNodesNotFound() throws Exception {
		client.runScriptOnNodesMatching(runningInGroup("zebras-are-awesome"),
	...
	public void testImageById() {
		Template defaultTemplate = view.getComputeService().templateBuilder()
				.build();
		assertEquals(
				view.getComputeService().getImage(
						defaultTemplate.getImage().getId()),
	...
	public void testAScriptExecutionAfterBootWithBasicTemplate()
		...
		try {
			Set<? extends NodeMetadata> nodes = client.createNodesInGroup(
					group, 1, template);
			NodeMetadata node = get(nodes, 0);
			LoginCredentials good = node.getCredentials();
			assert good.identity != null : nodes;

			for (Entry<? extends NodeMetadata, ExecResponse> response : client
					.runScriptOnNodesMatching(
							runningInGroup(group),
							"hostname",
							wrapInInitScript(false).runAsRoot(false)
									.overrideLoginCredentials(good)).entrySet()) {
				checkResponseEqualsHostname(response.getValue(),
						response.getKey());
			}

			// test single-node execution
			ExecResponse response = client.runScriptOnNode(node.getId(),
					"hostname", wrapInInitScript(false).runAsRoot(false));
			...
			// test adding AdminAccess later changes the default boot user, in
			// this
			// case to foo, with home dir /over/ridden/foo
			ListenableFuture<ExecResponse> future = client.submitScriptOnNode(
					node.getId(), AdminAccess.builder().adminUsername("foo")
							.adminHome("/over/ridden/foo").build(),
					nameTask("adminUpdate"));

			response = future.get(3, TimeUnit.MINUTES);

			assert response.getExitStatus() == 0 : node.getId() + ": "
					+ response;
			...
			response = client.runScriptOnNode(node.getId(), "echo $USER",
					wrapInInitScript(false).runAsRoot(false));

			assert response.getOutput().trim().equals("foo") : node.getId()
					+ ": " + response;
	...
	@Test(enabled = false)
	protected void tryBadPassword(String group, Credentials good)
			throws AssertionError {
		try {
			Map<? extends NodeMetadata, ExecResponse> responses = client
					.runScriptOnNodesMatching(
							runningInGroup(group),
							...
							wrapInInitScript(false).runAsRoot(false)
									.overrideLoginCredentials(
											LoginCredentials.builder()
													.user(good.identity)
													.noPrivateKey()
													.password("romeo").build()));
			assert responses.size() == 0 : "shouldn't pass with a bad password\n"
					+ responses;
		} catch (AssertionError e) {
			throw e;
		} catch (RunScriptOnNodesException e) {
			assert Iterables.any(e.getNodeErrors().values(),
					Predicates.instanceOf(AuthorizationException.class)) : e
					+ " not authexception!";
		}
	}

	@Test(enabled = false)
	public void weCanCancelTasks(NodeMetadata node)
			throws InterruptedException, ExecutionException {
		ListenableFuture<ExecResponse> future = client
				.submitScriptOnNode(node.getId(), "sleep 300",
		...
		try {
			...
			response = client.runScriptOnNode(node.getId(),
					"/tmp/init-sleeper status", wrapInInitScript(false)
							.runAsRoot(false));
			assert !response.getOutput().trim().equals("") : node.getId()
					+ ": " + response;
			future.cancel(true);
			response = client.runScriptOnNode(node.getId(),
					"/tmp/init-sleeper status", wrapInInitScript(false)
	...
	protected void checkResponseEqualsHostname(ExecResponse execResponse,
			NodeMetadata node1) {
		assert execResponse.getOutput().trim().equals(node1.getHostname()) : node1
				+ ": " + execResponse;
	}
	...
	public void testTemplateMatch() throws Exception {
		...
		Template toMatch = client.templateBuilder()
				.imageId(template.getImage().getId()).build();
	...
	public void testCreateTwoNodesWithRunScript() throws Exception {
		...
		try {
			...
			nodes = newTreeSet(concat(e.getSuccessfulNodes(), e.getNodeErrors()
					.keySet()));
			throw e;
		}
		...
		assertLocationSameOrChild(
				checkNotNull(node1.getLocation(), "location of %s", node1),
				template.getLocation());
		assertLocationSameOrChild(
				checkNotNull(node2.getLocation(), "location of %s", node2),
				template.getLocation());
	...
	public void testCreateTwoNodesWithOneSpecifiedName() throws Exception {
		...
		assertTrue(node1.getName().equals("first-node")
				|| node2.getName().equals("first-node"),
				"one node should be named 'first-node'");
		assertFalse(node1.getName().equals("first-node")
				&& node2.getName().equals("first-node"),
	...
	private Template refreshTemplate() {
		return template = addRunScriptToTemplate(buildTemplate(client
				.templateBuilder()));
	}
	...
	protected void checkOsMatchesTemplate(NodeMetadata node) {
		if (node.getOperatingSystem() != null)
			assert node
					.getOperatingSystem()
					.getFamily()
					.equals(template.getImage().getOperatingSystem()
							.getFamily()) : String.format(
					"expecting family %s but got %s", template.getImage()
							.getOperatingSystem().getFamily(),
	...
	public void testCreateAnotherNodeWithANewContextToEnsureSharedMemIsntRequired()
		...
		boolean existingLocationIsAssignable = Iterables.any(
				client.listAssignableLocations(),
				Predicates.equalTo(existingLocation));

		if (existingLocationIsAssignable) {
			getAnonymousLogger().info(
					"creating another node based on existing nodes' location: "
							+ existingLocation);
			template = buildTemplate(client.templateBuilder());
			template = addRunScriptToTemplate(client.templateBuilder()
		...
		Set<? extends NodeMetadata> nodes = client.createNodesInGroup(group, 1,
				template);
		...
		if (existingLocationIsAssignable)
			...
			this.assertLocationSameOrChild(
					checkNotNull(node.getLocation(), "location of %s", node),
					template.getLocation());
	...
	public void testConcurrentUseOfComputeServiceToCreateNodes()
		...
		ListeningExecutorService userExecutor = MoreExecutors
				.listeningDecorator(context.utils().userExecutor());
		...
		try {
			for (int i = 0; i < 2; i++) {
				...
				groups.add(group);
				template = buildTemplate(client.templateBuilder());
				template.getOptions().inboundPorts(22, 8080)
						.blockOnPort(22, 300 + groupNum);
				ListenableFuture<NodeMetadata> future = userExecutor
						.submit(new Callable<NodeMetadata>() {
							public NodeMetadata call() throws Exception {
								NodeMetadata node = getOnlyElement(client
										.createNodesInGroup(group, 1, template));
								getAnonymousLogger().info(
										"Started node " + node.getId());
								return node;
							}
						});
				futures.add(future);
			...
			ListenableFuture<List<NodeMetadata>> compoundFuture = Futures
					.allAsList(futures);
	...
	public void testCredentialsCache() throws Exception {
		...
		for (NodeMetadata node : nodes)
			assert view.utils().credentialStore().get("node#" + node.getId()) != null : "credentials for "
					+ node.getId();
	}
	...
	protected Map<? extends NodeMetadata, ExecResponse> runScriptWithCreds(
		...
		return client.runScriptOnNodesMatching(runningInGroup(group),
				InstallJDK.fromOpenJDK(), overrideLoginCredentials(creds)
						.nameTask("runScriptWithCreds"));
	}

	protected void checkNodes(Iterable<? extends NodeMetadata> nodes,
			String group, String taskName) throws IOException {
		for (NodeMetadata node : nodes) {
			assertNotNull(node.getProviderId());
			assertNotNull(node.getGroup());
			assertEquals(node.getGroup(), group);
			assertEquals(node.getStatus(), NodeMetadataStatus.RUNNING);
			Credentials fromStore = view.utils().credentialStore()
					.get("node#" + node.getId());
			assertEquals(fromStore, node.getCredentials());
			assert node.getPublicAddresses().size() >= 1
					|| node.getPrivateAddresses().size() >= 1 : "no ips in"
					+ node;
	...
	public void testGet() throws Exception {
		...
		for (NodeMetadata node : nodes) {
			...
			assertLocationSameOrChild(
					checkNotNull(metadata.getLocation(), "location of %s",
							metadata), template.getLocation());
			checkImageIdMatchesTemplate(metadata);
			checkOsMatchesTemplate(metadata);
			assert metadata.getStatus() == NodeMetadataStatus.RUNNING : metadata;
			// due to DHCP the addresses can actually change in-between runs.
			assertEquals(
					metadata.getPrivateAddresses().size(),
					node.getPrivateAddresses().size(),
					format("[%s] didn't match: [%s]", metadata
							.getPrivateAddresses(), node.getPrivateAddresses()
							.size()));
			assertEquals(
					metadata.getPublicAddresses().size(),
					node.getPublicAddresses().size(),
					format("[%s] didn't match: [%s]", metadata
							.getPublicAddresses(), node.getPublicAddresses()
							.size()));
		}
	...
	protected void assertNodeZero(Collection<? extends NodeMetadata> metadataSet) {
		assert metadataSet.size() == 0 : format(
	...
	public void testReboot() throws Exception {
		Set<? extends NodeMetadata> rebootNodes = client
				.rebootNodesMatching(inGroup(group));
	...
	public void testSuspendResume() throws Exception {
		...
		Set<? extends NodeMetadata> suspendedNodes = client
				.suspendNodesMatching(inGroup(group));
		...
		assert Iterables.all(stoppedNodes, new Predicate<NodeMetadata>() {
			...
			public boolean apply(NodeMetadata input) {
				boolean returnVal = input.getStatus() == NodeMetadataStatus.SUSPENDED;
				if (!returnVal)
					getAnonymousLogger().warning(
							format("node %s in state %s%n", input.getId(),
									input.getStatus()));
				return returnVal;
			}
		...
		Set<? extends NodeMetadata> resumedNodes = client
				.resumeNodesMatching(inGroup(group));
	...
	public void testListNodesByIds() throws Exception {
		...
		SortedSet<NodeMetadata> listedNodes = ImmutableSortedSet.copyOf(client
				.listNodesByIds(nodeIds));
	...
	public void testGetNodesWithDetails() throws Exception {
		for (NodeMetadata node : client.listNodesDetailsMatching(all())) {
			...
			if (nodeMetadata.getStatus() == NodeMetadataStatus.RUNNING) {
	...
	public void testDestroyNodes() {
		...
		Set<? extends NodeMetadata> destroyed = client
				.destroyNodesMatching(inGroup(group));
		assertEquals(toDestroy, destroyed.size());
		for (NodeMetadata node : filter(client.listNodesDetailsMatching(all()),
				inGroup(group))) {
			assert node.getStatus() == NodeMetadataStatus.TERMINATED : node;
	...
	static class ServiceStats {
		...
		public String toString() {
			return format(
					"[backgroundProcessMilliseconds=%s, socketOpenMilliseconds=%s]",
					backgroundProcessMilliseconds, socketOpenMilliseconds);
		}
	}

	protected ServiceStats trackAvailabilityOfProcessOnNode(Statement process,
			String processName, NodeMetadata node) {
		ServiceStats stats = new ServiceStats();
		Stopwatch watch = Stopwatch.createStarted();
		ExecResponse exec = client.runScriptOnNode(node.getId(), process,
				runAsRoot(false).wrapInInitScript(false));
		stats.backgroundProcessMilliseconds = watch
				.elapsed(TimeUnit.MILLISECONDS);
		watch.reset().start();

		HostAndPort socket = null;
		try {
			socket = openSocketFinder.findOpenSocketOnNode(node, 8080, 600,
					TimeUnit.SECONDS);
		} catch (NoSuchElementException e) {
		...
		getAnonymousLogger().info(
				format("<< %s on node(%s)[%s] %s", processName, node.getId(),
						socket, stats));
		return stats;
	}
	...
	protected void createAndRunAServiceInGroup(String group)
			throws RunNodesException {
		// note that some cloud providers do not support mixed case tag names
		ImmutableMap<String, String> userMetadata = ImmutableMap
				.<String, String> of("test", group);

		ImmutableSet<String> tags = ImmutableSet.of(group);
		Stopwatch watch = Stopwatch.createStarted();

		template = buildTemplate(client.templateBuilder());
		template.getOptions().inboundPorts(22, 8080).blockOnPort(22, 300)
				.userMetadata(userMetadata).tags(tags);
		...
		getAnonymousLogger().info(
				format("<< configured node(%s) with %s and jetty %s in %ss",
						nodeId, exec(nodeId, "java -fullversion"),
						exec(nodeId, JettyStatements.version()),
						configureSeconds));

		trackAvailabilityOfProcessOnNode(JettyStatements.start(),
				"start jetty", node);

		client.runScriptOnNode(nodeId, JettyStatements.stop(), runAsRoot(false)
	...
	protected String exec(final String nodeId, Statement command) {
		return client
				.runScriptOnNode(nodeId, command,
	...
	protected void checkUserMetadataContains(NodeMetadata node,
		...
		Map<String, String> missing = Maps.difference(node.getUserMetadata(),
				userMetadata).entriesOnlyOnRight();
		assert missing.isEmpty() : format(
				"node userMetadata did not contain %s %s", missing, node);
	}

	protected void checkTagsInNodeEquals(NodeMetadata node,
			ImmutableSet<String> tags) {
		assert node.getTags().equals(tags) : format(
	...
	public void testGetAssignableLocations() throws Exception {
		for (Location location : client.listAssignableLocations()) {
			...
			case ZONE:
				...
				break;
			case HOST:
				Location provider2 = location.getParent().getParent()
						.getParent();
				// zone can be a direct descendant of provider
				if (provider2 == null)
	...
	public void testOptionToNotBlock() throws Exception {
		...
		try {
			...
			assert duration < nonBlockDurationSeconds : format(
					"duration(%d) longer than expected(%d) seconds! ",
	...
	public void testCompareSizes() throws Exception {
		...
		Hardware smallest = client.templateBuilder().smallest().build()
				.getHardware();
		Hardware fastest = client.templateBuilder().fastest().build()
				.getHardware();
		Hardware biggest = client.templateBuilder().biggest().build()
				.getHardware();
		...
		assert getCores(smallest) <= getCores(fastest) : format("%s ! <= %s",
				smallest, fastest);
		assert getCores(biggest) <= getCores(fastest) : format("%s ! <= %s",
				biggest, fastest);

		assert biggest.getRam() >= fastest.getRam() : format("%s ! >= %s",
				biggest, fastest);
		assert biggest.getRam() >= smallest.getRam() : format("%s ! >= %s",
				biggest, smallest);

		assert getCores(fastest) >= getCores(biggest) : format("%s ! >= %s",
				fastest, biggest);
		assert getCores(fastest) >= getCores(smallest) : format("%s ! >= %s",
				fastest, smallest);
	}
	...
	protected void doCheckJavaIsInstalledViaSsh(NodeMetadata node,
		...
		try {
			...
			assert exec.getError().indexOf("OpenJDK") != -1
					|| exec.getOutput().indexOf("OpenJDK") != -1 : exec
					+ "\n"
					+ ssh.exec("cat /tmp/" + taskName + "/" + taskName
							+ ".sh /tmp/" + taskName + "/stdout.log /tmp/"
							+ taskName + "/stderr.log");
		} finally {
			if (ssh != null)
				ssh.disconnect();
	...
	protected void tearDownContext() {
		try {
			if (nodes != null) {
				...
				client.destroyNodesMatching(Predicates.<NodeMetadata> or(
						inGroup(group), in(nodes)));
			}
		} catch (Exception e) {
