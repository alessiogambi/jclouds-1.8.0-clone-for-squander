...
public class TemplateBuilderImplTest {
	...
	public void testMultiMaxNull() {
		...
		assertEquals(TemplateBuilderImpl.multiMax(Ordering.natural()
				.nullsLast(), values), Arrays.asList((Object) null));
	}

	public void testMultiMaxNulls() {
		Iterable<String> values = Arrays.asList("1", "3", null, "2", "2", null);
		assertEquals(TemplateBuilderImpl.multiMax(Ordering.natural()
				.nullsLast(), values), Arrays.asList((Object) null, null));
	}

	protected Location provider = new LocationBuilder()
			.scope(LocationScope.PROVIDER).id("aws-ec2").description("aws-ec2")
			.build();

	protected Location region = new LocationBuilder()
			.scope(LocationScope.REGION).id("us-east-1")
			.description("us-east-1").parent(provider).build();

	protected Location region2 = new LocationBuilder()
			.scope(LocationScope.REGION).id("us-east-2")
			.description("us-east-2").parent(provider).build();

	protected OperatingSystem os = OperatingSystem.builder().name("osName")
			.version("osVersion").description("osDescription").arch("X86_32")
			.build();
	...
	protected Image image = new ImageBuilder()
			.id(getProviderFormatId("imageId")).providerId("imageId")
			.name("imageName").description("imageDescription")
			.version("imageVersion").operatingSystem(os)
			.status(ImageStatus.AVAILABLE).location(region).build();

	protected Image image64bit = ImageBuilder.fromImage(image)
			.id(getProviderFormatId("image64bId")).providerId("image64bId")
			.operatingSystem(os.toBuilder().arch("X86_64").build()).build();

	protected Image imageArchNull = ImageBuilder.fromImage(image)
			.id(getProviderFormatId("imageArchNullId"))
			.providerId("imageArchNullId")
			.operatingSystem(os.toBuilder().arch(null).build()).build();

	protected Image imageNameAlt = ImageBuilder.fromImage(image)
			.id(getProviderFormatId("imageNameAltId"))
			.providerId("imageNameAltId").name("alternateImageName").build();
	...
	public void testLocationPredicateWhenComputeMetadataIsNotLocationBound() {
		...
		Supplier<Set<? extends Location>> locations = Suppliers
				.<Set<? extends Location>> ofInstance(ImmutableSet
						.<Location> of(region));
		Supplier<Set<? extends Image>> images = Suppliers
				.<Set<? extends Image>> ofInstance(ImmutableSet
						.<Image> of(image));
		Supplier<Set<? extends Hardware>> hardwares = Suppliers
				.<Set<? extends Hardware>> ofInstance(ImmutableSet
		...
		TemplateBuilderImpl template = createTemplateBuilder(null, locations,
				images, hardwares, region, optionsProvider,
	...
	/**
	 * Resolves images using a default TemplateBuilder, optionally applying a
	 * customizer to that TemplateBuilder where the customizer may e.g. apply an
	 * imageChooser
	 */
	@SuppressWarnings("unchecked")
	protected void doTestResolveImagesWithTemplateBuilderCustomizer(
		...
		TemplateBuilderImpl template = createTemplateBuilder(null, locations,
				inputImages, hardwares, region, optionsProvider,
				templateBuilderProvider, getImageStrategy);
	...
	protected void doTestResolveImages(
		...
		doTestResolveImagesWithTemplateBuilderCustomizer(inputImages,
				Functions.<TemplateBuilderImpl> identity(), expectedBest);
	}

	public void testResolveImagesSimple() {
		doTestResolveImages(
				Suppliers.<Set<? extends Image>> ofInstance(ImmutableSet
						.<Image> of(image, image64bit)), image64bit);
	}

	public void testResolveImagesPrefersNull() {
		// preferring null has been the default behaviour; not sure if this is
		// ideal
		// (would make more sense to prefer nonNull) but don't change behaviour
		// by default
		doTestResolveImages(
				Suppliers.<Set<? extends Image>> ofInstance(ImmutableSet
						.<Image> of(image, imageArchNull, image64bit)),
				imageArchNull);
	}
	...
	public void testResolveImagesCustomSorterPreferringNonNull() {
		...
		final Ordering<Image> sorterPreferringNonNullArch = new Ordering<Image>() {
			...
			public int compare(Image left, Image right) {
				return ComparisonChain
						...
						.compare(left, right,
								TemplateBuilderImpl.DEFAULT_IMAGE_ORDERING)
						.result();
			}
		};
		assertTrue(TemplateBuilderImpl.DEFAULT_IMAGE_ORDERING.compare(
				image64bit, imageArchNull) < 0, "wrong default image ordering");
		assertTrue(
		...
		Function<TemplateBuilderImpl, TemplateBuilderImpl> tbCustomiser = new Function<TemplateBuilderImpl, TemplateBuilderImpl>() {
			...
			public TemplateBuilderImpl apply(TemplateBuilderImpl input) {
				return input.imageChooser(input
						.imageChooserFromOrdering(sorterPreferringNonNullArch));
			}
		};
	...
	public void testResolveImagesPrefersImageBecauseNameIsLastAlphabetically() {
		// preferring that which comes later alphabetically is the default
		// behaviour;
		// see comments in TemplateBuilderImpl.DEFAULT_IMAGE_ORDERING
		doTestResolveImages(
				Suppliers.<Set<? extends Image>> ofInstance(ImmutableSet
						.<Image> of(imageNameAlt, image)), image);
	}
	...
	public void testResolveImagesCustomSorterPreferringAltImage() {
		Function<TemplateBuilderImpl, TemplateBuilderImpl> tbSortWithAlternate = new Function<TemplateBuilderImpl, TemplateBuilderImpl>() {
			...
			public TemplateBuilderImpl apply(TemplateBuilderImpl input) {
				return input.imageChooser(input
						.imageChooserFromOrdering(new Ordering<Image>() {
		...
		doTestResolveImagesWithTemplateBuilderCustomizer(
				Suppliers.<Set<? extends Image>> ofInstance(ImmutableSet
						.<Image> of(imageNameAlt, image, imageArchNull,
								image64bit)), tbSortWithAlternate, imageNameAlt);
	}
	...
	public void testResolveImagesCustomChooserPreferringAltImage() {
		Function<TemplateBuilderImpl, TemplateBuilderImpl> tbChooseAlternate = new Function<TemplateBuilderImpl, TemplateBuilderImpl>() {
			...
			public TemplateBuilderImpl apply(TemplateBuilderImpl input) {
				return input
						.imageChooser(new Function<Iterable<? extends Image>, Image>() {
							@Override
							public Image apply(Iterable<? extends Image> input) {
								return Iterables.find(input,
										new Predicate<Image>() {
											@Override
		...
		doTestResolveImagesWithTemplateBuilderCustomizer(
				Suppliers.<Set<? extends Image>> ofInstance(ImmutableSet
						.<Image> of(imageNameAlt, image, imageArchNull,
								image64bit)), tbChooseAlternate, imageNameAlt);
	}
	...
	public void testArchWins() {
		...
		assertEquals(template.smallest().osArchMatches("X86_32").build()
				.getImage(), image);
	...
	public void testHardwareWithImageIdPredicateOnlyAcceptsImage() {
		...
		Hardware hardware = new HardwareBuilder()
				...
				.supportsImage(
						ImagePredicates
								.idEquals(getProviderFormatId("imageId")))
				.build();
	...
	public void testOptionsUsesDefaultTemplateBuilder() {
		...
		TemplateBuilderImpl template = createTemplateBuilder(null, locations,
				images, hardwares, defaultLocation, optionsProvider,
				templateBuilderProvider, getImageStrategy);
	...
	protected TemplateBuilderImpl createTemplateBuilder(Image knownImage,
		...
		TemplateBuilderImpl template = new TemplateBuilderImpl(locations,
				new ImageCacheSupplier(images, 60), hardwares,
				Suppliers.ofInstance(defaultLocation), optionsProvider,
				templateBuilderProvider, getImageStrategy);
		return template;
	}
	...
	public void testSuppliedImageLocationWiderThanDefault() {
		...
		assertEquals(
				template.imageId(image.getId()).locationId(provider.getId())
						.build().getLocation(), region);
	...
	public void testSuppliedLocationAndOptions() {
		...
		try {
			template.imageId("foo").options(provideTemplateOptions())
					.locationId("location").build();
			fail("Expected NoSuchElementException");
		} catch (NoSuchElementException e) {
	...
	public void testDefaultLocationWithNoOptionsNoSuchElement() {
		...
		expect(optionsProvider.get()).andReturn(defaultOptions);
		expect(getImageStrategy.getImage(anyObject(String.class))).andReturn(
				null);
	...
	public void testDefaultLocationWithUnmatchedPredicateExceptionMessageAndLocationNotCalled() {
		...
		try {
			...
			assertEquals(
					e.getMessage(),
					"no image matched predicate: Predicates.and(nullEqualToIsParentOrIsGrandparentOfCurrentLocation(),imageDescription(notDescription))");
		}
	...
	public void testDefaultLocationWithOptions() {
		...
		try {
			template.imageId("region/ami").options(provideTemplateOptions())
					.build();
			fail("Expected NoSuchElementException");
		} catch (NoSuchElementException e) {
	...
	public void testFindImageWithIdDefaultToGetImageStrategy() {
		final Supplier<Set<? extends Location>> locations = Suppliers
				.<Set<? extends Location>> ofInstance(ImmutableSet
						.<Location> of(region));
		...
		final Supplier<Set<? extends Hardware>> hardwares = Suppliers
				.<Set<? extends Hardware>> ofInstance(ImmutableSet.<Hardware> of(new HardwareBuilder()
						.ids(String
								.format("datacenter(%s)platform(%s)cpuCores(%d)memorySizeMB(%d)diskSizeGB(%d)",
										"Falkenberg", "Xen", 1, 512, 5))
						.ram(512)
						.processors(ImmutableList.of(new Processor(1, 1.0)))
						.volumes(
								ImmutableList.<Volume> of(new VolumeImpl(
										(float) 5, true, true)))
						.hypervisor("Xen").location(region)
		...
		Provider<TemplateBuilder> templateBuilderProvider = new Provider<TemplateBuilder>() {
			...
			public TemplateBuilder get() {
				return createTemplateBuilder(null, locations, images,
						hardwares, region, optionsProvider, this,
						getImageStrategy);
			}
		};
	...
	public void testFindImageWithIdDefaultToGetImageStrategyAndPopulatesTheCache() {
		...
		try {
			// First call searching for the image properties will fail, as the
			// image is not in the cache
			templateBuilder.osNameMatches(image.getOperatingSystem().getName())
					.build();
		...
		// A second call using the imageId will fallback to the GetImageStrategy
		// and populate the image in the cache.
		assertNotNull(templateBuilder.imageId(image.getId()).build());

		// The third call will succeed, as the previous one should have
		// populated the image in the cache.
		templateBuilder.imageId(null); // Clear all criteria
		Template template = templateBuilder.osNameMatches(
				image.getOperatingSystem().getName()).build();
		assertEquals(template.getImage().getId(), image.getId());

		// Verify this is called only once, as the third call will already find
	...
	public void testMatchesHardwareWithIdPredicate() {
		...
		final Supplier<Set<? extends Hardware>> hardwares = Suppliers
				.<Set<? extends Hardware>> ofInstance(ImmutableSet.<Hardware> of(
						new HardwareBuilder()
								...
								.ram(512)
								.processors(
										ImmutableList.of(new Processor(1, 1.0)))
								.volumes(
										ImmutableList
												.<Volume> of(new VolumeImpl(
														(float) 5, true, true)))
								.hypervisor("Xen")
								.location(region)
								.supportsImage(
										ImagePredicates.idIn(ImmutableSet
												.of("Ubuntu 11.04 x64")))
								.build(),
						new HardwareBuilder()
								.ids(String
										.format("datacenter(%s)platform(%s)cpuCores(%d)memorySizeMB(%d)diskSizeGB(%d)",
												"Falkenberg", "OpenVZ", 1, 512,
												5))
								.ram(512)
								.processors(
										ImmutableList.of(new Processor(1, 1.0)))
		...
		TemplateBuilder templateBuilder = templateBuilderProvider.get()
				.minRam(512).osFamily(OsFamily.UBUNTU)
				.hypervisorMatches("OpenVZ").osVersionMatches("1[10].[10][04]")
				.os64Bit(true);
	...
	public void testImageLocationNonDefault() {
		...
		TemplateBuilder templateBuilder = templateBuilderProvider.get()
				.hardwareId("m1.small").imageId("us-east-2/ami-ffff");

		assertEquals(templateBuilder.toString(),
				"{imageId=us-east-2/ami-ffff, hardwareId=m1.small}");
	...
	public void testFromSpecWithLoginUser() {
		...
		TemplateBuilder templateBuilder = templateBuilderProvider
				.get()
				.from("hardwareId=m1.small,imageId=us-east-2/ami-ffff,loginUser=user:Password01,authenticateSudo=true");
		...
		assertEquals(template.getOptions().shouldAuthenticateSudo(),
				Boolean.TRUE);
	}
}
