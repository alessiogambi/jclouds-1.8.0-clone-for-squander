...
public class PersistNodeCredentialsTest {
	...
	public void testReturnsCorrectFunction() {
		PersistNodeCredentials persistNodeCredentials = Guice.createInjector(
				new PersistNodeCredentialsModule(), new AbstractModule() {
		...
		assertEquals(
				persistNodeCredentials.ifAdminAccess(null).getClass(),
	...
	public void testRefreshCredentialsForNodeIfRanAdminAccessWhenStatementIsNullSameCredentialsAndNoCaching() {
		...
		NodeMetadata node = new NodeMetadataBuilder().ids("id")
	...
	public void testRefreshCredentialsForNodeWhenStatementIsNullSameCredentialsAndDoesCache() {
		...
		NodeMetadata node = new NodeMetadataBuilder().ids("id")
				.status(NodeMetadataStatus.RUNNING).credentials(credentials)
				.build();
		RefreshCredentialsForNode fn = new PersistNodeCredentialsModule.RefreshCredentialsForNode(
				credstore, null);
		assertEquals(node, fn.apply(node));
	...
	public void testRefreshCredentialsForNodeIfRanAdminAccessWhenStatementIsAdminAccessNewCredentialsAndDoesCache() {
		...
		LoginCredentials credentials = LoginCredentials.builder().user("foo")
				.build();

		expect(statement.getAdminCredentials()).andReturn(credentials)
				.atLeastOnce();
		expect(credstore.put("node#id", credentials)).andReturn(null);
