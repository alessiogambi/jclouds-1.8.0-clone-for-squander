...
import static org.jclouds.compute.domain.NodeMetadataStatus.RUNNING;
...
public class ConcurrentOpenSocketFinderTest {
	...
	private final NodeMetadata node = new NodeMetadataBuilder().id("myid")
			.status(RUNNING).publicAddresses(ImmutableSet.of(PUBLIC_IP))
	...
	public void testRespectsTimeout() throws Exception {
		...
		OpenSocketFinder finder = new ConcurrentOpenSocketFinder(
				socketAlwaysClosed, nodeRunning, userExecutor);
		...
		assertTrue(timetaken >= timeoutMs - EARLY_GRACE
				&& timetaken <= timeoutMs + SLOW_GRACE, "timetaken="
				+ timetaken);
	...
	public void testReturnsReachable() throws Exception {
		...
		OpenSocketFinder finder = new ConcurrentOpenSocketFinder(
				secondSocketOpen, nodeRunning, userExecutor);
	...
	public void testChecksSocketsConcurrently() throws Exception {
		ControllableSocketOpen socketTester = new ControllableSocketOpen(
				ImmutableMap.of(HostAndPort.fromParts(PUBLIC_IP, 22),
						new SlowCallable<Boolean>(true, 1500),
						HostAndPort.fromParts(PRIVATE_IP, 22),
						new SlowCallable<Boolean>(true, 1000)));

		OpenSocketFinder finder = new ConcurrentOpenSocketFinder(socketTester,
				nodeRunning, userExecutor);
	...
	public void testAbortsWhenNodeNotRunning() throws Exception {
		...
		OpenSocketFinder finder = new ConcurrentOpenSocketFinder(
				socketAlwaysClosed, nodeNotRunning, userExecutor) {
			@Override
			protected <T> Predicate<T> retryPredicate(
					final Predicate<T> findOrBreak, long timeout, long period,
					TimeUnit timeUnits) {
	...
	public void testSocketFinderAllowedInterfacesAll() throws Exception {
		FluentIterable<String> ips = ConcurrentOpenSocketFinder
				.checkNodeHasIps(node, AllowedInterfaces.ALL);
	...
	public void testSocketFinderAllowedInterfacesPrivate() throws Exception {
		FluentIterable<String> ips = ConcurrentOpenSocketFinder
				.checkNodeHasIps(node, AllowedInterfaces.PRIVATE);
	...
	public void testSocketFinderAllowedInterfacesPublic() throws Exception {
		FluentIterable<String> ips = ConcurrentOpenSocketFinder
				.checkNodeHasIps(node, AllowedInterfaces.PUBLIC);
		assertTrue(ips.contains(PUBLIC_IP));
	...
	private static class ControllableSocketOpen implements SocketOpen {
		...
		ControllableSocketOpen(
				Map<HostAndPort, ? extends Callable<Boolean>> answers) {
